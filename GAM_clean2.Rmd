...
```{r}
pca_soil_all <- prcomp(df_all[21:26], scale=TRUE, center=TRUE)
...
#run GAM
gam_model <- gam(Average.infiltration.rate ~ Drainage.Area.Ratio + s(Age) + s(Percent.Vegetation.Shrub) + s(Percent.Vegetation.Other) + Engineered.Soil.Percent.Sand + s(Soil_PC_1) + s(Soil_PC_2) + Engineered.Soil.Percent.Compost 
...
columns <- df_res_removed[21:26]

get_gam(df_res_removed, x, x_lin, y, columns, 12, 0.2)
```
```{r}
